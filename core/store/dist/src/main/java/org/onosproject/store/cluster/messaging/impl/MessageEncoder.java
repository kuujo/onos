...
// had to specify <Object> to avoid Class Loader not being able to find some classes.
    ...
    public MessageEncoder(Endpoint endpoint, Version version, int preamble) {
        super();
    ...
    private void encodeMessage(InternalMessage message, ByteBuf out) {
        // If the header hasn't been written to the channel, write it.
        if (!headerWritten) {
            // Write the sender IP.
            ...
            byte[] versionBytes = version.toString().getBytes(StandardCharsets.UTF_8);
