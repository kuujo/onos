...
public class AtomixConsistentMap<K, V> implements AsyncConsistentMap<K, V> {
    private final io.atomix.core.map.AsyncConsistentMap<K, V> atomixMap;
    private final Map<MapEventListener<K, V>, io.atomix.core.map.MapEventListener<K, V>> listenerMap =
        Maps.newIdentityHashMap();

    public AtomixConsistentMap(io.atomix.core.map.AsyncConsistentMap<K, V> atomixMap) {
    ...
    public CompletableFuture<Collection<Versioned<V>>> values() {
        return atomixMap.values()
            .thenApply(values -> values.stream().map(this::toVersioned).collect(Collectors.toList()));
    }

    @Override
    public CompletableFuture<Set<Map.Entry<K, Versioned<V>>>> entrySet() {
        return atomixMap.entrySet()
            .thenApply(entries -> entries.stream()
                .map(entry -> Maps.immutableEntry(entry.getKey(), toVersioned(entry.getValue())))
    ...
    public synchronized CompletableFuture<Void> addListener(MapEventListener<K, V> listener, Executor executor) {
        io.atomix.core.map.MapEventListener<K, V> atomixListener = event ->
        ...
        future.whenComplete((result, error) -> {
            if (error == null) {
                ...
                if (cause instanceof io.atomix.primitive.PrimitiveException.ConcurrentModification) {
                    newFuture.completeExceptionally(new ConsistentMapException.ConcurrentModification(error.getMessage()));
