...
public class Transaction<T> {
    ...
    enum State {
         ...
         * concurrently running transactions depending on the transaction's {@link IsolationLevel}.
         */
        ACTIVE,
    ...
    private final Transactional<T> transactionalObject;
    ...
    public CompletableFuture<LockVersion> begin() {
        open();
        return transactionalObject.begin();
    }
     ...
     * {@link TransactionException} if a concurrent modification is detected for an {@link IsolationLevel} that does
    ...
    public CompletableFuture<Boolean> prepare(List<T> updates) {
        ...
        // Create and submit the transaction log. Note that we do not provide the lock version since weaker
        // isolation levels only use key-level locks. Version checks will occur only for writes on specific keys.
        return transactionalObject.prepare(transactionId, updates).thenApply(succeeded -> {
    ...
    public CompletableFuture<Boolean> prepareAndCommit(List<T> updates) {
        ...
        // Create and submit the transaction log. Note that we do not provide the lock version since weaker
        // isolation levels only use key-level locks. Version checks will occur only for writes on specific keys.
        return transactionalObject.prepareAndCommit(transactionId, updates).thenApply(succeeded -> {
