...
public class StorageManager implements StorageService, StorageAdminService {
    ...
    public List<MapInfo> getMapInfo() {
        ...
        return atomix.getPrimitives(ConsistentMapType.instance())
            .stream()
            .map(info -> {
                io.atomix.core.map.ConsistentMap<String, byte[]> map =
                    atomix.<String, byte[]>consistentMapBuilder(info.name())
    ...
    public Map<String, WorkQueueStats> getQueueStats() {
        ...
        return atomix.getPrimitives(WorkQueueType.instance())
            .stream()
            .map(info -> {
                io.atomix.core.queue.WorkQueue queue = atomix.workQueueBuilder(info.name())
                    .withSerializer(new AtomixSerializerAdapter(serializer))
                    .build();
