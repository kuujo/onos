...
/**
 * Map transaction participant that uses the {@link org.onosproject.store.service.IsolationLevel#READ_COMMITTED}
 * isolation level.
 */
public class ReadCommittedMapParticipant<K, V> extends TransactionalMapParticipant<K, V> {
    private final Map<K, Versioned<V>> readCache = Maps.newConcurrentMap();

    public ReadCommittedMapParticipant(AsyncConsistentMap<K, V> backingMap, Transaction<MapRecord<K, V>> transaction) {
        super(backingMap, transaction);
    }

    @Override
    protected V read(K key) {
        Versioned<V> value = backingConsistentMap.getOrDefault(key, null);
