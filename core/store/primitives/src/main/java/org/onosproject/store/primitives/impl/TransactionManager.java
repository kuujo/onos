...
public class TransactionManager {
    ...
    <K, V> PartitionedTransactionalMap<K, V> getTransactionalMap(
        ...
        Map<PartitionId, TransactionalMapPartition<K, V>> partitions = new HashMap<>();
            ...
            try {
                AsyncTransactionalMap<K, V> baseMap =
                        (AsyncTransactionalMap<K, V>) partitionService.getDistributedPrimitiveCreator(partitionId)
                                .newAsyncConsistentMap(name, serializer);
                AsyncConsistentMap<K, V> asyncMap = mapCache.get(name, () ->
                        DistributedPrimitives.newCachingMap(
                                DistributedPrimitives.newMeteredMap(baseMap)));
                partitions.put(partitionId, new TransactionalMapPartition<>(name, asyncMap, new Transaction<>(
