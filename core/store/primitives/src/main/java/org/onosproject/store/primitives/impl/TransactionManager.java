...
public class TransactionManager {
    ...
    private <K, V> TransactionalMapParticipant<K, V> getTransactionalMapPartition(
        ...
        try {
            AsyncTransactionalMap<K, V> baseMap =
                    (AsyncTransactionalMap<K, V>) partitionService.getDistributedPrimitiveCreator(partitionId)
            ...
            AsyncConsistentMap<K, V> asyncMap = mapCache.get(mapName, () ->
                    DistributedPrimitives.newCachingMap(
                            DistributedPrimitives.newMeteredMap(baseMap)));

            Transaction<MapRecord<K, V>> transaction = new Transaction<>(
                    transactionCoordinator.transactionId,
                    transactionCoordinator.isolationLevel,
                    baseMap);
            switch (transactionCoordinator.isolationLevel) {
                case SERIALIZABLE:
                    return new SerializableMapParticipant<>(asyncMap, transaction);
                case SNAPSHOT:
                    return new SnapshotMapParticipant<>(asyncMap, transaction);
                case REPEATABLE_READ:
                    return new RepeatableReadMapParticipant<>(asyncMap, transaction);
                case READ_COMMITTED:
                case READ_UNCOMMITTED:
                    return new ReadCommittedMapParticipant<>(asyncMap, transaction);
                default:
