 ...
 */
public class AtomixConsistentMapState extends ResourceStateMachine implements SessionListener {
    ...
    private final Map<TransactionId, TransactionScope> transactions = Maps.newHashMap();
    ...
    protected MapEntryUpdateResult<String, byte[]> updateAndGet(Commit<? extends UpdateAndGet> commit) {
        try {
            ...
            Versioned<byte[]> newMapValue = newValue == null ? null
                    : new Versioned<>(newValue, version);
    ...
    /**
     * Handles a transaction begin commit.
     *
    ...
    protected long begin(Commit<? extends TransactionBegin> commit) {
        transactions.put(commit.operation().transactionId(), new TransactionScope(commit));
        return commit.index();
    }
    ...
    protected PrepareResult prepareAndCommit(Commit<? extends TransactionPrepareAndCommit> commit) {
        ...
        TransactionScope transactionScope = transactions.remove(commit.operation().transactionId());
    ...
    protected PrepareResult prepare(Commit<? extends TransactionPrepare> commit) {
        ...
        TransactionScope transactionScope = transactions.get(commit.operation().transactionId());
        if (transactionScope == null) {
            log.warn("Unknown transaction {}", commit.operation().transactionId());
            return PrepareResult.PARTIAL_FAILURE;
        }

        try {
            List<MapRecord<String, byte[]>> transactionLog = commit.operation().transactionLog();

            // Iterate through records in the transaction log and perform isolation checks.
            for (MapRecord<String, byte[]> record : transactionLog) {
                ...
                if (record.type() == MapRecord.Type.VERSION_MATCH && key == null) {
                ...
                if (existingValue == null) {
                    ...
                    if (record.currentVersion() != transactionScope.version()) {
                        return PrepareResult.OPTIMISTIC_LOCK_FAILURE;
                    }
                } else {
                    // If the value is non-null, compare the current version with the record version.
                    if (existingValue.version() > record.currentVersion()) {
                        return PrepareResult.OPTIMISTIC_LOCK_FAILURE;
                    }
                }
            }

            // No violations detected. Add to pendingTransactions and mark modified keys locked for transactions.
            transactions.put(commit.operation().transactionId(), transactionScope.prepared(commit));
            transactionLog.forEach(record -> {
                if (record.type() != MapRecord.Type.VERSION_MATCH) {
    ...
    protected CommitResult commit(Commit<? extends TransactionCommit> commit) {
        ...
        TransactionScope transactionScope = transactions.remove(transactionId);
        if (transactionScope == null) {
    ...
    private CommitResult commit(TransactionScope transactionScope) {
        List<MapRecord<String, byte[]>> transactionLog = transactionScope.transactionLog();
        boolean retainTombstones = !transactions.isEmpty();
        ...
        long totalReferencesToCommit = transactionLog.stream()
                ...
                .filter(record -> record.type() != MapRecord.Type.VERSION_MATCH && record.type() != MapRecord.Type.LOCK
                        && (record.type() != MapRecord.Type.REMOVE_IF_VERSION_MATCH || retainTombstones))
                .count();
        ...
        for (MapRecord<String, byte[]> record : transactionLog) {
            ...
            if (record.type() == MapRecord.Type.LOCK) {
                continue;
            }
            ...
            if (record.type() != MapRecord.Type.REMOVE_IF_VERSION_MATCH) {
                newValue = new TransactionalCommit(version, record.value(), completer);
            }
            // For deletes, if tombstones need to be retained then create and store a tombstone commit.
            else if (retainTombstones) {
                newValue = new TombstoneCommit(version, completer);
            }
    ...
    protected RollbackResult rollback(Commit<? extends TransactionRollback> commit) {
        ...
        if (transactionScope == null) {
            ...
            try {
                transactionScope.transactionLog()
                        .forEach(record -> {
                            if (record.type() != MapRecord.Type.VERSION_MATCH) {
    ...
    private void discardTombstones() {
        if (transactions.isEmpty()) {
            ...
            long lowWaterMark = transactions.values().stream()
    ...
    private MapEntryUpdateResult.Status validate(UpdateAndGet update) {
        ...
        Long existingVersion = existingValue == null ? null : existingValue.version();
     ...
     */
    private static class TransactionScope {
        private final Commit<? extends TransactionBegin> beginCommit;
        private final Commit<? extends TransactionPrepare> prepareCommit;
        private final Commit<? extends TransactionCommit> commitCommit;

        TransactionScope(Commit<? extends TransactionBegin> beginCommit) {
            this(beginCommit, null);
        }

        private TransactionScope(
                Commit<? extends TransactionBegin> beginCommit,
                Commit<? extends TransactionPrepare> prepareCommit) {
            this(beginCommit, prepareCommit, null);
        }
        ...
        private TransactionScope(
            ...
            this.beginCommit = checkNotNull(beginCommit);
        ...
        TransactionScope prepared(Commit<? extends TransactionPrepare> commit) {
            return new TransactionScope(beginCommit, commit);
        }
        ...
        TransactionScope committed(Commit<? extends TransactionCommit> commit) {
            checkState(isPrepared());
            return new TransactionScope(beginCommit, prepareCommit, commit);
        }
